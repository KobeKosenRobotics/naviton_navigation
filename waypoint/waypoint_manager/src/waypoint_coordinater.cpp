...
void WaypointCoordinater::publish()
{
    ...
    try
    {
        geometry_msgs::TransformStamped transform = tf_buffer->lookupTransform("base_link", "map", ros::Time(0), ros::Duration(1.0));
...
void WaypointCoordinater::publish()
{
    ...
    try {
        geometry_msgs::TransformStamped transform = tf_buffer.lookupTransform("naviton/base_link", "map", ros::Time(0));
    ...
    // 相対位置を計算
    geometry_msgs::Pose relative_pose = calculateRelativePosition(waypoint_to_calculate.pose.pose, current_pose);
